...
`verifyExactSolutionV2()`は解法列をソルバー内部規則で初期状態から再生する。
...
高速探索のため、ソルバーはスライドルールをtyped array上へ実装している。正本`src/core/rules.js`との将来のずれを検出するため、返却解法を独立した`applySlide()`でも再生する差分試験を追加する。
...
- [x] GitHub ActionsのNode Gateが成功した。
- [x] Browser Gateが成功した。
- [ ] 人間レビューが完了する。
