...
async function expectPhase2Bootstraps(page) {
  await expect.poll(() => page.evaluate(() => ({
    countdown: Boolean(window.hakodase?.__p203Installed),
    playing: Boolean(window.hakodase?.__p204Installed),
    result: Boolean(window.hakodase?.__p205Installed),
  }))).toEqual({ countdown: true, playing: true, result: true });
}

async function beginTransitionCapture(page) {
  await page.evaluate(() => {
    window.__hakodaseGateUnsubscribe?.();
    window.__hakodaseGateTransitions = [];
    window.__hakodaseGateUnsubscribe = window.hakodase.appController.subscribe((event) => {
      window.__hakodaseGateTransitions.push({
        state: event.state,
        playingHidden: document.getElementById('screenPlaying').hidden,
        timeText: document.getElementById('time').textContent,
      });
    });
  });
}

async function expectCountdownThenPlaying(page) {
  const transitions = await page.evaluate(() => window.__hakodaseGateTransitions || []);
  const countdownIndex = transitions.findIndex((entry) => entry.state === 'countdown');
  const playingIndex = transitions.findIndex((entry, index) => index > countdownIndex && entry.state === 'playing');
  expect(countdownIndex).toBeGreaterThanOrEqual(0);
  expect(playingIndex).toBeGreaterThan(countdownIndex);
  expect(transitions[countdownIndex].playingHidden).toBe(true);
  expect(transitions[countdownIndex].timeText).toBe('0.00秒');
}

async function startMode(page, mode = 'practice') {
  await page.goto('/');
  await expect(page.locator('body')).toHaveAttribute('data-app-state', 'home');
  await expectPhase2Bootstraps(page);
  ...
  await beginTransitionCapture(page);
  await page.locator('#nameForm button[type="submit"]').click();
  await expect(page.locator('body')).toHaveAttribute('data-app-state', 'playing', { timeout: 8_000 });
  await expectCountdownThenPlaying(page);
...
test('homeからnameConfirm、countdown、playingへ一度ずつ進む', async ({ page }) => {
  ...
  await expect(page.locator('#remaining')).toHaveText(/\d+ \/ \d+箱/);
  await expectNoHorizontalOverflow(page);
  expect(errors).toEqual([]);
});
